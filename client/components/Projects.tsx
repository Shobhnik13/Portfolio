          ...
          <div className="border-l-2 hover:border-purple-300  hover:duration-200 flex flex-col gap-y-1 p-4" key={data.id}>
            ...
            <div className="flex flex-row gap-x-4 text-sm 2xl:text-base text-gray-500/85">
              {data && data.status === 'Under Review' ? (
                ...
                </TooltipProvider>) : (data?.LiveLink !== '' && <Link className="flex gap-x-1 items-center justify-center hover:underline" target="_blank" href={`${data.LiveLink}`}>Live preview  <GoArrowUpRight className="text-base mt-1" /></Link>)}
              ...
              {data?.GithubLink !== '' && <Link className="flex gap-x-1 items-center justify-center hover:underline" target="_blank" href={`${data.GithubLink}`}>GitHub  <GoArrowUpRight className="text-base mt-1" /></Link>}
            </div>
