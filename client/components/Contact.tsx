      ...
      <div className="flex flex-col mt-8 gap-y-4">
        {socialData.map((data) => (
          <div key={data.id} className="flex flex-row gap-x-10">
            ...
            <Link className="text-sm text-slate-700 dark:text-slate-400 2xl:text-base text flex items-center justify-center gap-x-1 hover:underline text-gray-500/85" href={`${data.link}`} target="_blank">{data.subText} <GoArrowUpRight className="text-base mt-1" /></Link>
          </div>
        ))}
      </div>
    </div>
